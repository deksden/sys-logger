# Подсистема логирования (docs/logger/DOC_SYS_LOGGER.md, v0.7.0)
...
Если установлен `LOG_MAX_STRING_LENGTH` больше нуля, строковые значения в объектах
будут ограничены указанным количеством символов с добавлением маркера обрезки в конце.
Если `LOG_MAX_STRING_LENGTH=0` или не указан, ограничение длины строк не применяется.
...
Система может ограничивать длину строковых значений в объектах, что полезно при работе с большими объемами данных:
...
const longText = 'Очень длинный текст, который будет обрезан...'.repeat(10);
logger.info({ details: longText });
// Будет преобразовано в:
// { details: 'Очень длинный текст, который будет обрезан...Очень длинный текст, который будет обрезан...Очень длинный текст, который...' }
```

Если настройка `LOG_MAX_STRING_LENGTH=0` или не указана, ограничение длины строк не применяется.
...
5. **Производительность**:
    ...
    - Ограничивайте длину строк для больших объектов данных
